  ...
  <div class="min-h-[calc(100vh-4rem)] bg-gray-50 dark:bg-gray-950 transition-colors">
    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-indigo-600 via-purple-600 to-violet-700 py-16 px-4">
      ...
      <div class="relative max-w-4xl mx-auto text-center text-white">
        <p class="text-sm font-semibold uppercase tracking-widest text-white/70 mb-3">Contact</p>
        <h1 class="text-4xl sm:text-5xl font-bold mb-4">Get In Touch</h1>
        <p class="text-lg text-white/80 max-w-xl mx-auto">Have a question about an artwork, auction, or your account? We'd love to hear from you.</p>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div class="lg:col-span-3 page-card p-8">
          ...
          <form class="space-y-5" @submit.prevent="handleSubmit">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <BaseInput v-model="formData.name" label="Your Name" placeholder="John Doe" :error="errors.name" required />
              <BaseInput v-model="formData.email" type="email" label="Email Address" placeholder="you@example.com" :error="errors.email" required />
            </div>

            <BaseInput v-model="formData.subject" label="Subject" placeholder="How can we help?" :error="errors.subject" required />

            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Message <span class="text-red-500">*</span></label>
              <textarea v-model="formData.message" rows="5" class="form-textarea" placeholder="Tell us more about your inquiry..." required />
        ...
        <div class="lg:col-span-2 space-y-5">
          <div class="page-card p-6">
            <h3 class="font-bold text-gray-900 dark:text-white mb-5">Contact Information</h3>
            <div class="space-y-4">
              <div v-for="info in contactInfo" :key="info.label" class="flex items-start gap-4">
                <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" :class="info.bg">
                  <i :class="[info.icon, info.color]" />
                </div>
                <div>
                  <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ info.label }}</p>
                  <p class="text-sm text-gray-500 dark:text-gray-400 whitespace-pre-line">{{ info.value }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="page-card p-6">
            ...
            <div class="flex gap-3">
              <a v-for="social in socials" :key="social.label" href="#" :aria-label="social.label" class="social-btn" :class="social.class">
                <i :class="social.icon" />
              </a>
            </div>
          </div>
          ...
          <div class="page-card p-6 bg-gradient-to-br from-purple-600 to-indigo-600 border-0 text-white">
            ...
            <p class="text-sm text-white/80 mb-3">Our support team typically responds within 24 hours.</p>
            <a href="mailto:info@kelem.art" class="text-sm font-semibold underline underline-offset-2">info@kelem.art</a>
          </div>
        </div>
      </div>
    </div>
