  ...
  <div class="min-h-screen bg-gray-50 pt-20">
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="flex gap-2 mb-6">
        <button v-for="tab in tabs" :key="tab" @click="activeTab = tab" :class="['px-4 py-2 rounded-lg text-sm font-medium', activeTab === tab ? 'bg-purple-600 text-white' : 'bg-white text-gray-600']">{{ tab }}</button>
      </div>
      <div class="space-y-4">
        <div v-for="item in filtered" :key="item.id" class="bg-white rounded-xl shadow-md p-6 flex justify-between items-center">
          <div>
            <span class="text-xs uppercase text-gray-400">{{ item.type }}</span>
            <p class="font-bold">{{ item.title }}</p>
            <p class="text-sm text-gray-500">{{ item.submittedAt }}</p>
          </div>
          <div class="flex gap-2" v-if="item.status === 'pending'">
            <BaseButton size="sm" @click="approve(item.id)">Approve</BaseButton>
            <BaseButton size="sm" variant="outline" @click="reject(item.id)">Reject</BaseButton>
          </div>
          <span v-else class="text-sm capitalize" :class="item.status === 'approved' ? 'text-green-600' : 'text-red-600'">{{ item.status }}</span>
        </div>
      </div>
    </div>
  </div>
