  ...
  <div class="min-h-screen bg-gray-50 pt-20">
    <div class="container mx-auto px-4 py-8">
      ...
      <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        <ArtworkCard v-for="item in artworks" :key="item.id" :artwork="item" show-actions @view-details="$router.push(`/artworks/${item.id}`)" @add-to-cart="addToCart(item)" />
      </div>
      <EmptyState v-if="!loading && !artworks.length" title="No results" message="Try adjusting your filters." />
    </div>
  </div>
</template>
